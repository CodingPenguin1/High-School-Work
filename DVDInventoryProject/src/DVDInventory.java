...
public class DVDInventory {
	...
	public static void save(DVDRecord[] dvds, int dvdsIndex, String file) throws FileNotFoundException {
		PrintWriter writer = new PrintWriter(file);
		for (int i = 0; i <= dvdsIndex; i++) {
			writer.println(dvds[i].getTitle() + "▩" + dvds[i].getDirector() + "▩" + dvds[i].getGenre() + "▩" + dvds[i].getStar());
		}
		writer.close();
	}
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		DVDRecord[] dvds = new DVDRecord[100];
		int dvdsIndex = -1;
		...
		while (true) {
			...
			if (option == 1) {
				...
				while (reader.hasNext()) {
					...
					dvds[dvdsIndex] = new DVDRecord(title, director, genre, star);
				}
				reader.close();
